...
export default function MembersPage() {
  // ป้องกัน gangs.json ไม่ใช่ array
  const gangsData: Gang[] = Array.isArray(gangsRaw)
    ? (gangsRaw as unknown as Gang[])
    : Object.entries(gangsRaw).map(([slug, members]) => ({
        slug,
        name: slug.toUpperCase(),
        description: "",
        members: (members as { name: string }[]),
      }));
  ...
  return (
    ...
    >
      ...
      <header className="px-8 pt-12 pb-8 border-b border-zinc-900">
        ...
        <h1 className="text-8xl leading-none tracking-tight uppercase"
          style={{ fontFamily: "'Bebas Neue', sans-serif" }}>
          Members
        </h1>
        <p className="text-xs tracking-[0.3em] text-zinc-600 uppercase mt-4">{totalMembers} Total</p>
      </header>
        ...
        {gangsData.map((gang) => (
          <div key={gang.slug} className="mt-12">
            <div className="flex items-baseline gap-4 pb-4 border-b border-zinc-800">
              ...
              {gang.description && (
                <span className="text-xs tracking-[0.3em] text-red-600 uppercase">{gang.description}</span>
              )}
            ...
            {gang.members.map((member, idx) => (
              <div key={idx}
                ...
                <span className="text-3xl text-zinc-800 group-hover:text-red-600 transition-colors leading-none w-10 text-right flex-shrink-0"
                  style={{ fontFamily: "'Bebas Neue', sans-serif" }}>
                  {String(idx + 1).padStart(2, "0")}
                </span>
                <span className="text-lg font-bold tracking-widest uppercase flex-1">{member.name}</span>
                <span className="text-xs tracking-[0.3em] text-zinc-700 uppercase">{gang.slug}</span>
              </div>
            ))}
          </div>
        ))}
      ...
      <footer className="border-t border-zinc-900 px-8 py-5 flex items-center justify-between">
        <span className="text-xs tracking-[0.3em] text-zinc-800 uppercase">Born to be die for WRU</span>
        <Link href="/">
          <span className="text-xs tracking-[0.4em] text-zinc-700 uppercase"
            style={{ fontFamily: "'Bebas Neue', sans-serif" }}>← Home</span>
        </Link>
      </footer>
    </main>
  );
